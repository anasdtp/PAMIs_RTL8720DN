...
public:
    ...
    /// \param[in] StepPin1 Arduino digital pin number for motor pin 1. Defaults
    /// to pin 2. For a AccelStepper::DRIVER (interface==1), 
    /// this is the Step input to the driver. Low to high transition means to step)
    /// \param[in] DIRpin1 Arduino digital pin number for motor pin 2. Defaults
    /// to pin 3. For a AccelStepper::DRIVER (interface==1), 
    /// this is the Direction input the driver. High means forward.
    /// \param[in] StepPin2 Arduino digital pin number for motor pin 3. Defaults
    /// to pin 4.
    /// \param[in] DIRpin2 Arduino digital pin number for motor pin 4. Defaults
    /// to pin 5.
    /// \param[in] enable If this is true (the default), enableOutputs() will be called to enable
    /// the output pins at construction time.
    AccelStepper(uint8_t interface = AccelStepper::FULL2WIRE, uint8_t StepPin1 = 17, uint8_t DIRpin1 = 16, uint8_t StepPin2 = 19, uint8_t DIRpin2 = 18, bool enable = true);
    ...
    void setMouvement(MOUVEMENT mvt);
    ...
    void    runToNewPosition(long position, MOUVEMENT mvt);
